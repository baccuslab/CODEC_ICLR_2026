...
- Beyond the standard scientific stack, we rely heavily on 
  - `bscope`: for acquiring, saving, and manipulating the intermediate activations and contributions of a given model  
  - `fycus`: a little plotting library to help with visualization. This can simply be commented out. Replace all Fycus.save('...') calls with plt.imshow() and you can generate the same figures via standard means.
