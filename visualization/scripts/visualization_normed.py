...
for count, img_idx in enumerate(img_idxs):
    for WHICH_MODE in [0,1, 2, 3, 4, 5,6,7,8]: #, 5, 6, 7, -1, -2, -3, -4]:
        ...
        if count == 0:
            plt.plot(atom)
            plt.plot(np.sort(atom)[::-1], 'r')
            plt.title(f'Atom and Loadings for Layer {LAYER}, Class {CLASS}, Mode {WHICH_MODE} Idx {mode_idx}')
            plt.xlabel('Channel Index')
            plt.ylabel('Value')
            plt.axvline(x=CHANNELS, color='gray', linestyle='--', label=f'Top {CHANNELS} Channels')
            plt.legend(['Atom', 'Sorted Atom'])
            F.save(f'atom_loadings_layer_{LAYER}_class_{CLASS}_mode_{WHICH_MODE}', dpi=150)
            ...
            plt.axvline(x=CHANNELS, color='gray', linestyle='--', label=f'Top {CHANNELS} Channels')
            plt.legend(['Atom', 'Sorted Atom'])
            F.save(f'zoom_atom_loadings_layer_{LAYER}_class_{CLASS}_mode_{WHICH_MODE}', dpi=150)

            plt.plot(mode_loadings)
            plt.title(f'Mode Loadings for Layer {LAYER}, Class {CLASS}, Mode {WHICH_MODE} Idx {mode_idx}')
            plt.xlabel('Image Index')
            plt.ylabel('Loading Value')
            F.save(f'mode_loadings_layer_{LAYER}_class_{CLASS}_mode_{WHICH_MODE}', dpi=150)
